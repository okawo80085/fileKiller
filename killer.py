...
class KILLER():
	def __init__(self, targerLocation, *agents):
		self.Files = []
		self.Folders = []
		self.targerLocation = targerLocation
		self.kills = 0
	...
	def __repr__(self):
		return '<class killer(agent 69 if you will), kills={}, location={}, targets: nfiles={}, nfolders={}>'.format(self.kills, self.targerLocation, len(self.Files), len(self.Folders))

	def __call__(self):
		if os.path.exists(self.targerLocation) and os.path.isdir(self.targerLocation):
	...
	def pfr(self):
		...
		pathList = [i[0] for i in os.walk(self.targerLocation)]
